export const parse = function (str) {
    if (str[str.length - 1] !== '\n')
        str += '\n';
    const rLine = /^(\s*)(?:([\w\-]+)\s*:|(-))\s*(.*)/,
        rJSON = /^(?:\d+(?:\.\d+)?|null|true|false|"(?:[^"\x00-\x19\\]|\\(?:[rnt"]|u[\da-fA-F]{4}))*")$/;
    return accepts(0);

    function accepts() {

    }


    var ret = {};

    consume(0, -1, ret);
    return ret;

    function consume(index, current_indent, target) {
        var length = 0, first = true, isMap = false;
        while (1) {
            if (index === str.length) return index;
            var lineEnd = str.indexOf('\n', index), line = str.slice(index, lineEnd);
            if (/^\s*$/.test(line)) { // empty line or comment
                index = lineEnd + 1;
                continue;
            }
            var m = rLine.exec(line);
            if (!m)
                throw new Error('bad line: ' + line);
            var indent = m[1].length, name = m[2], val = m[4];
            if (indent <= current_indent)
                return index;
            if (first) {
                first = false;
                isMap = !m[3];
            } else if (!m[3] !== isMap) {
                throw new Error('bad line: ' + line);
            }

            index = lineEnd + 1;
            if (isMap) {
                // console.log(indent, name, val);
                if (val)
                    target[name] = rJSON.test(val) ? JSON.parse(val) : val;
                else
                    index = consume(index, indent, target[name] = {});
            } else {
                if (val)
                    target[length++] = rJSON.test(val) ? JSON.parse(val) : val;
                else
                    index = consume(index, indent, target[length++] = {});
                target.length = length;
            }
        }

    }
};

export const stringify = function (obj) {
    if (!obj || typeof obj !== 'object')
        throw new Error('');
    return stringify(obj, '');
    function stringify(obj, prefix) {
        switch (typeof obj) {
            case 'number':
        }

    }
};